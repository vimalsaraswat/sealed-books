//! Journal entry repository: SQLite persistence and conversion to/from `sealed_books_core::types::Entry`.

use crate::db::error::DbError;
use libsql::{Connection, params};
use sealed_books_core::types::{Direction, Entry, Line};

/// Extension trait providing database queries for `Entry`.
#[allow(async_fn_in_trait)]
pub trait EntryExt {
    /// Persists this entry and all of its balanced lines in a single database transaction.
    ///
    /// Validates double-entry balance, verifies that the target period is not sealed,
    /// and ensures all referenced account IDs exist.
    async fn post_to(&self, conn: &Connection, period_id: &str) -> Result<(), DbError>;

    /// Loads all entries and lines for a specific accounting period,
    /// returned in deterministic order (by entry timestamp, then entry ID, lines by line ID).
    async fn find_by_period(conn: &Connection, period_id: &str) -> Result<Vec<Entry>, DbError>;

    /// Loads a single entry by its unique ID.
    async fn find_by_id(conn: &Connection, id: &str) -> Result<Entry, DbError>;
}

impl EntryExt for Entry {
    async fn post_to(&self, conn: &Connection, period_id: &str) -> Result<(), DbError> {
        // 1. Validate double-entry balance and at least 1 debit and credit
        self.validate()?;

        // 2. Check that target period exists and is open
        let mut rows = conn
            .query(
                "SELECT status FROM periods WHERE id = ?1;",
                params![period_id],
            )
            .await?;

        let period_status: String = if let Some(row) = rows.next().await? {
            row.get(0)?
        } else {
            return Err(DbError::PeriodNotFound(period_id.to_string()));
        };

        if period_status == "sealed" {
            return Err(DbError::PeriodSealed(period_id.to_string()));
        }

        // 3. Verify all account IDs exist in the chart of accounts
        for line in &self.lines {
            let mut acc_rows = conn
                .query(
                    "SELECT 1 FROM accounts WHERE id = ?1;",
                    params![line.account_id.as_str()],
                )
                .await?;

            if acc_rows.next().await?.is_none() {
                return Err(DbError::AccountNotFound(line.account_id.clone()));
            }
        }

        // 4. Insert entry header and all lines
        conn.execute(
            "INSERT INTO entries (id, period_id, date, description, created_at)
             VALUES (?1, ?2, ?3, ?4, ?5);",
            params![
                self.id.as_str(),
                period_id,
                self.date.as_str(),
                self.description.as_str(),
                self.created_at.as_str(),
            ],
        )
        .await?;

        for line in &self.lines {
            let dir_str = match line.direction {
                Direction::Debit => "debit",
                Direction::Credit => "credit",
            };

            conn.execute(
                "INSERT INTO lines (id, entry_id, account_id, direction, amount_minor, description)
                 VALUES (?1, ?2, ?3, ?4, ?5, ?6);",
                params![
                    line.id.as_str(),
                    self.id.as_str(),
                    line.account_id.as_str(),
                    dir_str,
                    line.amount_minor as i64,
                    line.description.as_deref(),
                ],
            )
            .await?;
        }

        Ok(())
    }

    async fn find_by_period(conn: &Connection, period_id: &str) -> Result<Vec<Entry>, DbError> {
        let mut rows = conn
            .query(
                "SELECT id, date, description, created_at
                 FROM entries
                 WHERE period_id = ?1
                 ORDER BY created_at ASC, id ASC;",
                params![period_id],
            )
            .await?;

        let mut entry_headers = Vec::new();
        while let Some(row) = rows.next().await? {
            entry_headers.push((
                row.get::<String>(0)?,
                row.get::<String>(1)?,
                row.get::<String>(2)?,
                row.get::<String>(3)?,
            ));
        }

        let mut entries = Vec::new();
        for (id, date, description, created_at) in entry_headers {
            let lines = load_lines_for_entry(conn, &id).await?;
            entries.push(Entry {
                id,
                date,
                description,
                created_at,
                lines,
            });
        }

        Ok(entries)
    }

    async fn find_by_id(conn: &Connection, id: &str) -> Result<Entry, DbError> {
        let mut rows = conn
            .query(
                "SELECT id, date, description, created_at
                 FROM entries
                 WHERE id = ?1;",
                params![id],
            )
            .await?;

        let (entry_id, date, description, created_at) = if let Some(row) = rows.next().await? {
            (
                row.get::<String>(0)?,
                row.get::<String>(1)?,
                row.get::<String>(2)?,
                row.get::<String>(3)?,
            )
        } else {
            return Err(DbError::EntryNotFound(id.to_string()));
        };

        let lines = load_lines_for_entry(conn, &entry_id).await?;

        Ok(Entry {
            id: entry_id,
            date,
            description,
            created_at,
            lines,
        })
    }
}

/// Helper function to retrieve all lines for an entry in deterministic order (by line ID).
async fn load_lines_for_entry(conn: &Connection, entry_id: &str) -> Result<Vec<Line>, DbError> {
    let mut rows = conn
        .query(
            "SELECT id, account_id, direction, amount_minor, description
             FROM lines
             WHERE entry_id = ?1
             ORDER BY id ASC;",
            params![entry_id],
        )
        .await?;

    let mut lines = Vec::new();
    while let Some(row) = rows.next().await? {
        let id: String = row.get(0)?;
        let account_id: String = row.get(1)?;
        let dir_str: String = row.get(2)?;
        let amount_minor: i64 = row.get(3)?;
        let description: Option<String> = row.get(4)?;

        let direction = match dir_str.as_str() {
            "debit" => Direction::Debit,
            "credit" => Direction::Credit,
            _ => {
                return Err(DbError::Sqlite(libsql::Error::Misuse(
                    "Invalid direction in database".into(),
                )));
            }
        };

        lines.push(Line {
            id,
            account_id,
            direction,
            amount_minor: amount_minor as u64,
            description,
        });
    }

    Ok(lines)
}

#[cfg(test)]
mod tests {
    use super::*;
    use crate::db::repository::account::Account;
    use crate::db::repository::period::PeriodRecord;
    use crate::db::schema::migrate;
    use sealed_books_core::hash_entry;

    async fn setup_test_db() -> Connection {
        let db = libsql::Builder::new_local(":memory:")
            .build()
            .await
            .unwrap();
        let conn = db.connect().unwrap();
        conn.execute("PRAGMA foreign_keys = ON;", ()).await.unwrap();
        migrate(&conn).await.unwrap();
        conn.execute(
            "INSERT OR IGNORE INTO organizations (id, name, base_currency, created_at) VALUES ('org_acme', 'Acme', 'USD', '2026-08-01T00:00:00Z');",
            (),
        ).await.unwrap();
        conn
    }

    fn sample_period() -> PeriodRecord {
        PeriodRecord {
            id: "per_aug_2026".into(),
            organization_id: "org_acme".into(),
            entity: "Acme Trading Pvt Ltd".into(),
            start_date: "2026-08-01".into(),
            end_date: "2026-08-31".into(),
            status: "open".into(),
        }
    }

    async fn seed_accounts(conn: &Connection) {
        Account {
            id: "acc_bank".into(),
            organization_id: "org_acme".into(),
            code: "1010".into(),
            name: "Bank Checking".into(),
            account_type: "asset".into(),
        }
        .insert(conn)
        .await
        .unwrap();

        Account {
            id: "acc_sales".into(),
            organization_id: "org_acme".into(),
            code: "4010".into(),
            name: "Sales Revenue".into(),
            account_type: "revenue".into(),
        }
        .insert(conn)
        .await
        .unwrap();
    }

    #[tokio::test]
    async fn test_save_to_db_and_reload_preserves_canonical_hash() {
        let conn = setup_test_db().await;
        seed_accounts(&conn).await;
        let period = sample_period();
        period.insert(&conn).await.unwrap();

        let original_entry = Entry {
            id: "ent_001".into(),
            date: "2026-08-15".into(),
            description: "Online product sale".into(),
            created_at: "2026-08-15T10:00:00Z".into(),
            lines: vec![
                Line::new("l_01", "acc_bank", Direction::Debit, 5000, None).unwrap(),
                Line::new("l_02", "acc_sales", Direction::Credit, 5000, None).unwrap(),
            ],
        };

        let original_hash = hash_entry(&original_entry).unwrap();

        original_entry.post_to(&conn, &period.id).await.unwrap();

        let loaded_entry = Entry::find_by_id(&conn, "ent_001").await.unwrap();
        let loaded_hash = hash_entry(&loaded_entry).unwrap();

        assert_eq!(
            original_hash, loaded_hash,
            "Re-loaded database entry hash must match original in-memory hash exactly"
        );
    }

    #[tokio::test]
    async fn test_reject_unbalanced_entry() {
        let conn = setup_test_db().await;
        seed_accounts(&conn).await;
        let period = sample_period();
        period.insert(&conn).await.unwrap();

        let unbalanced = Entry {
            id: "ent_bad".into(),
            date: "2026-08-15".into(),
            description: "Unbalanced".into(),
            created_at: "2026-08-15T10:00:00Z".into(),
            lines: vec![
                Line::new("l_01", "acc_bank", Direction::Debit, 5000, None).unwrap(),
                Line::new("l_02", "acc_sales", Direction::Credit, 4000, None).unwrap(),
            ],
        };

        let res = unbalanced.post_to(&conn, &period.id).await;
        assert!(res.is_err());
        assert!(matches!(res.unwrap_err(), DbError::Core(_)));
    }

    #[tokio::test]
    async fn test_reject_posting_to_sealed_period() {
        let conn = setup_test_db().await;
        seed_accounts(&conn).await;
        let mut period = sample_period();
        period.status = "sealed".into();
        period.insert(&conn).await.unwrap();

        let entry = Entry {
            id: "ent_002".into(),
            date: "2026-08-15".into(),
            description: "Sale".into(),
            created_at: "2026-08-15T10:00:00Z".into(),
            lines: vec![
                Line::new("l_01", "acc_bank", Direction::Debit, 5000, None).unwrap(),
                Line::new("l_02", "acc_sales", Direction::Credit, 5000, None).unwrap(),
            ],
        };

        let res = entry.post_to(&conn, &period.id).await;
        assert!(res.is_err());
        assert!(matches!(res.unwrap_err(), DbError::PeriodSealed(_)));
    }

    #[tokio::test]
    async fn test_reject_nonexistent_account() {
        let conn = setup_test_db().await;
        let period = sample_period();
        period.insert(&conn).await.unwrap();

        let entry = Entry {
            id: "ent_003".into(),
            date: "2026-08-15".into(),
            description: "Invalid account".into(),
            created_at: "2026-08-15T10:00:00Z".into(),
            lines: vec![
                Line::new("l_01", "acc_nonexistent", Direction::Debit, 5000, None).unwrap(),
                Line::new("l_02", "acc_sales", Direction::Credit, 5000, None).unwrap(),
            ],
        };

        let res = entry.post_to(&conn, &period.id).await;
        assert!(res.is_err());
        assert!(matches!(res.unwrap_err(), DbError::AccountNotFound(_)));
    }
}
